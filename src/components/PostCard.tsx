...
const PostCard: React.FC<PostCardProps> = ({ title, author, date, permalink, excerpt, category, tags }) => {
  ...
  const ExcerptDisplay = () => {
    ...
    return (
      <div className="font-light text-sm leading-8 text-black dark:text-white" dangerouslySetInnerHTML={{__html: excerptText}} />
    )
  }

  return (
    <article className={`relative bg-white dark:bg-gray-800 rounded shadow w-full h-auto p-4 flex flex-col justify-between items-start`}>
      <span className={classNames("p-2 uppercase bg-gray-300 text-black font-semibold text-xs rounded-sm mb-2", {
        'bg-yellow-500': primaryCategory === 'Guest Post',
      })}>
        {primaryCategory}
      </span>
      <h3 className="font-bold text-xl text-black dark:text-white mb-1">{title}</h3>
